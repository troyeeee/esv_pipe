...
def main():
    ...
    for line in open(args.vcf_file):
        ...
        if "INV" in record[4]:
            ...
            record[1] = r3
            record[2] = ID + "_" + r1+":2"
            record[3] = str(fa[record[0]][c_pos + sv_len-1:c_pos + sv_len+2][0])
            record[4] = alt3
            out_put.write('\t'.join(record))
            # 4
            record[1] = r4
            record[2] = ID + "_" + r2+":2"
            record[3]= str(fa[record[0]][c_pos+sv_len:c_pos+sv_len+2][0])
            record[4] = alt4
            out_put.write('\t'.join(record))
        if "DEL" in record[4]:
            r1 = record[1]
            r2 = str(c_pos + sv_len + 1)
